...
class Kube:
    ...
    def deleteResources(self,label):
        ...
        # delete job
        print "Found {} jobs matching {}...".format(len(job_details.items),label)
        for item in job_details.items:
            ...
            print("-- Deployment {} deleted. status='{}' \n".format(name ,api_response.status))
        print " \n"

        # delete service
        print "Found {} services matching {}...".format(len(service_details.items),label)
        for item in service_details.items:
            name = item.metadata.name
            print "attempting to delete service {}".format(name)
            ...
            body=client.V1DeleteOptions(
                propagation_policy='Foreground',
                grace_period_seconds=5))
            print("Service {} deleted. status='{}' \n".format(name ,api_response.status))
        print " \n"

        # delete deployment
        print "Found {} deployments matching {}...".format(len(deployment_details.items),label)
        for item in deployment_details.items:
            name = item.metadata.name
            print "attempting to delete deployment {}".format(name)
